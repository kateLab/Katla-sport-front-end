...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number) {
    ...
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, true).subscribe(c => requiredHiveSection.isDeleted = true);
  }

  onUndelete(hiveSectionId: number) {
    var requiredHiveSection = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, false).subscribe(c => requiredHiveSection.isDeleted = false);
  }
}
